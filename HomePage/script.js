...
function showPanel(panel){
  ...
  if(pLogin) pLogin.style.display=panel==='login'?'':'none';
  if(pSignup) pSignup.style.display=panel==='signup'?'':'none';
  setTimeout(clearAuthFields,0);
}

if(loginBtn) loginBtn.addEventListener('click',()=>openAuth('login'));
if(signupBtn) signupBtn.addEventListener('click',e=>{e.preventDefault();openAuth('signup');});
...
function activateProfile(name){
  ...
  if(pInitial) pInitial.textContent=initial;
  const nameItem=document.getElementById('profileNameItem');
  if(nameItem)nameItem.textContent='Signed in as '+name.trim();
}

function goToAnalyzer(e) {
  if (e) e.preventDefault();
  if (isLoggedIn) {
    window.location.href = 'http://localhost:5001/?user=' + encodeURIComponent(loggedInName.trim());
  } else {
...
function goToInterview(e) {
  if (e) e.preventDefault();
  if (isLoggedIn) {
    window.location.href = 'http://localhost:5002/?user=' + encodeURIComponent(loggedInName.trim());
  } else {
    // Force them to login first
    openAuth('login');
  }
...
function goToBuilder(e) {
  if (e) e.preventDefault();
  if (isLoggedIn) {
    window.location.href = 'http://localhost:5004/?user=' + encodeURIComponent(loggedInName.trim());
  } else {
    // Force them to login first
    openAuth('login');
  }
...
async function selectDemoAccount(idx){
  ...
  if(idx < accounts.length){
    ...
    closeAuth();
    activateProfile(accounts[idx].name);
  } else {
...
if(logoutBtn) {
  logoutBtn.addEventListener('click',e=>{
    ...
    const pInitial = document.getElementById('profileInitial');
    if(pInitial) pInitial.textContent='?';
    const nameItem=document.getElementById('profileNameItem');
